        ...
        let(:not_found_response) {
          {
            status: 404,
            headers: { 'Content-Type' => 'application/json' },
            body: JSON.dump(error_message: 'Product does not exist',
                            param: nil)
          }
        }

        it 'raises a RecordNotFound exception' do
          _request_stub = WebMock.stub_request(:get, record_path_regex)
                                 .to_return(not_found_response)
          expect {
            adapter.fetch(dummy_id)
          }.to raise_error(Ordoro::Record::RecordNotFound)
        end
      end
    end
    ...
    describe '#build' do
      ...
      context 'when attributes are provided' do
        let(:random_attribute) {
          record_class.attribute_set.find { |att|
            att.type == Axiom::Types::String
          }.name
        }
